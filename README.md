...
Linux compilation command:
    ...
    # cc -std=c11 -O2 -g -o dbtest standalone.c db*.c artree/*.c btree1/*.c -lpthread

Sample single thread output from indexing 40M pennysort keys:

    # ./dbtest tstdb -cmds=w -noDocs -keyLen=10 pennykey0-3
    ...
    -rw-rw-r-- 1 guest engr    1048576 Oct 18 09:16 tstdb
    -rw-rw-r-- 1 guest engr 2147483648 Oct 18 09:16 tstdb.ARTreeIdx

Sample four thread output from indexing 40M pennysort keys:

    # ./dbtest tstdb -cmds=w -noDocs -keyLen=10 pennykey[0123]
...
Sample four thread output from finding 40M pennysort keys:
    $ ./dbtest tstdb -cmds=f -keyLen=10 -idxType=0 -noDocs pennykey[0123]
...
Sample single thread output from indexing 80M pennysort keys:
    ...
    # ./dbtest tstdb -cmds=w -noDocs -keyLen=10 pennykey0-7
...
Sample eight thread output from indexing 80M pennysort keys:
    ...
    # ./dbtest tstdb -cmds=w -noDocs -keyLen=10 pennykey[01234567]
    started indexing for pennykey0
    started indexing for pennykey1
    started indexing for pennykey2
    started indexing for pennykey3
    ...
    -rw-rw-r-- 1 guest engr    1048576 Oct 18 06:22 tstdb
    -rw-rw-r-- 1 guest engr 4294967296 Oct 18 06:22 tstdb.ARTreeIdx

Sample output from finding 80M pennysort records:

    $ ./dbtest tstdb -cmds=f -keyLen=10 -noDocs pennykey[01234567]
    started finding keys for pennykey1
    started finding keys for pennykey2
    started finding keys for pennykey0
...
Sample output from storing/indexing/persisting 40M pennysort records (4GB):
    ...
    # ./dbtest tstdb -cmds=w -keyLen=10 penny0-3
    ...
    -rw-rw-r-- 1 guest guest     1048576 Oct 18 06:57 tstdb
    -rw-rw-r-- 1 guest guest 17179869184 Oct 18 07:01 tstdb.documents
    -rw-rw-r-- 1 guest guest  4294967296 Oct 18 07:01 tstdb.documents.ARTreeIdx

Sample output with four concurrent threads each storing 10M pennysort records:

    # ./dbtest tstdb -cmds=w -keyLen=10 -idxType=1 penny[0123]
    ...
    -rw-r--r-- 1 guest engr    1048576 Sep 16 22:15 tstdb
    -rw-r--r-- 1 guest engr 8589934592 Sep 16 22:16 tstdb.documents
    -rw-r--r-- 1 guest engr 2147483648 Sep 16 22:16 tstdb.documents.Btree1Idx
...
Sample cursor scan with min and max values:
    ...
    $ ./dbtest tstdb -cmds=s -minKey=aaaA -maxKey=aaaK -noDocs
...
Sample 4 thread indexing and finding of 40M keys into a btree index:
    ...
    $ ./dbtest tstdb -cmds=w -keyLen=10 -idxType=1 -noDocs pennykey[0123]
    started indexing for pennykey0
    started indexing for pennykey1
    started indexing for pennykey2
    started indexing for pennykey3
    ...
    $ ./dbtest tstdb -cmds=f -keyLen=10 -idxType=1 -noDocs pennykey[0123]
    started finding keys for pennykey0
    started finding keys for pennykey1
    started finding keys for pennykey2
    started finding keys for pennykey3
